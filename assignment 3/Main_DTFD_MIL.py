...
parser.add_argument('--mDATA0_dir_train0', default='Dataset/Camelyon16/mDATA_train.pkl', type=str)  ## Train Set
parser.add_argument('--mDATA0_dir_val0', default='Dataset/Camelyon16/mDATA_test.pkl', type=str)      ## Validation Set
parser.add_argument('--mDATA_dir_test0', default='Dataset/Camelyon16/mDATA_test.pkl', type=str)         ## Test Set
...
def main():
    ...
    SlideNames_train, FeatList_train, ClusterList_train, Label_train = reOrganize_mDATA(mDATA_train)
    SlideNames_val, FeatList_val, ClusterList_val, Label_val = reOrganize_mDATA(mDATA_val)
    SlideNames_test, FeatList_test, ClusterList_test, Label_test = reOrganize_mDATA_test(mDATA_test)
    ...
    for ii in range(params.EPOCH):
        ...
        train_attention_preFeature_DTFD(classifier=classifier, dimReduction=dimReduction, attention=attention, UClassifier=attCls, mDATA_list=(SlideNames_train, FeatList_train, ClusterList_train, Label_train), ce_cri=ce_cri,
        ...
        auc_val = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                                           UClassifier=attCls, mDATA_list=(SlideNames_val, FeatList_val, ClusterList_val, Label_val), criterion=ce_cri, epoch=ii,  params=params, f_log=log_file, writer=writer, numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)
        print_log(' ', log_file)
        ...
        tauc = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                                        UClassifier=attCls, mDATA_list=(SlideNames_test, FeatList_test, ClusterList_test, Label_test), criterion=ce_cri, epoch=ii,  params=params, f_log=log_file, writer=writer, numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)
        print_log(' ', log_file)
...
def test_attention_DTFD_preFeat_MultipleMean(mDATA_list, classifier, dimReduction, attention, UClassifier, epoch, criterion=None,  params=None, f_log=None, writer=None, numGroup=3, total_instance=3, distill='MaxMinS'):
    ...
    SlideNames, FeatLists, ClusterLists, Label = mDATA_list
    ...
    with torch.no_grad():
        ...
        for idx in tqdm(range(numIter), desc='Testing'):
            ...
            for tidx, tfeat in enumerate(batch_feat):
                ...
                for jj in range(params.num_MeanInference):
                    ...
                    # SAM Semantic Clustering Replacement
                    index_chunk_list = []
                    for k in range(numGroup):
                        idx_k = (tcluster_tensor == k).nonzero(as_tuple=True)[0].tolist()
                        if len(idx_k) == 0:
                            idx_k = [random.randint(0, tfeat.shape[0]-1)]
                        index_chunk_list.append(idx_k)
                ...
                loss1 = F.nll_loss(torch.log(allSlide_pred_softmax + 1e-5), tslideLabel)
...
def train_attention_preFeature_DTFD(mDATA_list, classifier, dimReduction, attention, UClassifier,  optimizer0, optimizer1, epoch, ce_cri=None, params=None,
    ...
    SlideNames_list, mFeat_list, ClusterList_list, Label_dict = mDATA_list
    ...
    for idx in tqdm(range(numIter), desc='Training'):
        ...
        for tidx, (tslide, slide_idx) in enumerate(zip(tslide_name, tidx_slide)):
            ...
            tcluster_tensor = ClusterList_list[slide_idx].to(params.device)
            ...
            for k in range(numGroup):
                idx_k = (tcluster_tensor == k).nonzero(as_tuple=True)[0].tolist()
                if len(idx_k) == 0:
                    idx_k = [random.randint(0, tfeat_tensor.shape[0]-1)]
                index_chunk_list.append(idx_k)
...
def reOrganize_mDATA_test(mDATA):
    ...
    for slide_name in mDATA.keys():
        ...
        label = patch_data_list[0]['label']
        Label.append(label)

        featGroup = []
        clusterGroup = []
        for tpatch in patch_data_list:
            if isinstance(tpatch['feature'], torch.Tensor):
                tfeat = tpatch['feature']
            else:
                tfeat = torch.from_numpy(tpatch['feature'])
            featGroup.append(tfeat.unsqueeze(0))
            if 'cluster_id' in tpatch:
                clusterGroup.append(tpatch['cluster_id'])
            else:
                clusterGroup.append(0)
        featGroup = torch.cat(featGroup, dim=0) ## numPatch x fs
        clusterGroup = torch.tensor(clusterGroup)
        FeatList.append(featGroup)
        ClusterList.append(clusterGroup)

    return SlideNames, FeatList, ClusterList, Label
